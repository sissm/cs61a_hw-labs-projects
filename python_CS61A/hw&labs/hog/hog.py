...
def roll_dice(num_rolls, dice=six_sided):
    ...
    if has1:
        ...
        return score
    "dice() 会返回一次掷骰结果；但 dice（不带括号）本身只是函数对象。"
    "每写一次 dice() 就会重新掷一次，不是复用上一次结果。"
...
def boar_brawl(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 2
    a = opponent_score %100 // 10
    b=player_score %100%10
    score=3*abs(a-b)
    if a-b==0:
        score=1
    return score
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    # END PROBLEM 9
    count=1
    m = make_averaged(roll_dice, times_called)
    max_num=m(1,dice)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    if sus_update(0,score,opponent_score,six_sided)>=(threshold+score):
        return 0
    else:
        return num_rolls
    # Remove this line once implemented.
